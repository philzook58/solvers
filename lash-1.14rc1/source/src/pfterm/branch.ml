(* Lash *)
(* ported from Satallax file: pfterm/branch.ml *)
...
module Branch =
	...
	let add (b,h,_) l = 
		let t = loc_literal_to_trm l in  if debug_search then Printf.printf  "adding (%d) to the branch\n" l ;
		...
		if IntSet.mem (-l) !b 
			...
			raise (Closed (Conflict(loc_literal_to_trm(abs l),loc_literal_to_trm(-(abs l))),Dependency.make [l;-l]) ) )
	...
	let is_taut l = let t = loc_literal_to_trm (-l) in 
