(* Lash *)
(* ported from Satallax file: pfterm/coq.ml *)
...
let rec ref_coq1 c r hyp const sp = raise (Failure "not in Lash")
let ref_coq c r = raise (Failure "not in Lash")
...
let ref_coq c r = 
  ...
  let con =
    match !conjecture with
      [] -> False
    | [(con,_)] -> coqnorm con
    | _ -> raise (Failure "ref_coq cannot handle more than one conjecture")
  in
  (* initialise hypotheses *)
  let hyp = List.fold_left (fun l (s,pt) -> (coqnorm pt,s)::l ) [] !coqsig_hyp_trm in
  let h1 = get_hyp_name() in
  Printf.fprintf c "\ntab_start %s.\n" h1;
  ref_coq1 c r ((fneg con,h1)::hyp) (!coqsig_const) ""; 
	      ...
	      let h2 = get_hyp_name() in
	        begin
	          match pt with
		          ...
		          | Lam(a1,Ap(Ap(Imp,Ap(Ap(Imp,DB(0,_)),False)),False)) when is_ar a1 && ty_get_l a1 = mk_prop && ty_get_r a1 = mk_prop ->
...
let ref_isabellehol c r = raise (Failure "not in Lash")
...
let rec print_stp_tstp c m p =
  if is_base m then
    Printf.fprintf c "%s" (no_basename (ty_get_no m))
...
let rec trm_to_tstp c m bound =
  ...
  with Not_found ->
    ...
    | Ap(Imp,m1) -> trm_to_tstp c (Lam(mk_prop,Ap(Ap(Imp,shift m1 0 1),DB(0,mk_prop)))) bound
    | Imp -> trm_to_tstp c (Lam(mk_prop,Lam(mk_prop,Ap(Ap(Imp,DB(1,mk_prop)),DB(0,mk_prop))))) bound
...
let tstp_axioms_f : (int,string * trm * out_channel * (bool ref)) Hashtbl.t = Hashtbl.create 100;;
...
(*	let fst = ref true in*)
	...
	begin
	  ...
	  | Lam(a1,Ap(Ap(Imp,Ap(Ap(Imp,DB(0,_)),False)),False)) when is_ar a1 && ty_get_l a1 = mk_prop && ty_get_r a1 = mk_prop ->  rewout "tab_rew_dn"
...
let ref_tstp c r =
  ...
  match !conjecture with
  | [(con,_)] ->
     let ccon = ftm_trm name_tp [] con in
     let ccon = coqnorm ccon in (*conjecture*)
      begin
	if let (b,_) = tstp_enough_subformulas ccon 0 in b then (* Chad, May 3 2016: Some problems have large conjectures that cause significant time to be wasted printing the same subformulas multiple times; in these cases create definitions sPn. *)
      ...
      let cname =
        begin
          match !conjecturenames with
          | [] -> "claim"
          | [n] -> n
          | _ -> raise (Failure "no proof output when multiple conjectures")
        end
      in
      Printf.fprintf c "thf(%s,conjecture," cname;
      flush c;
  ...
  | _ -> raise (Failure "tstp cannot handle multiple conjectures")
...
let rec coq_sterm c m bound lp rp =
  ...
  | Ap(Imp,m1) -> coq_sterm c (Lam(mk_prop,Ap(Ap(Imp,shift m1 0 1),DB(0,mk_prop)))) bound lp rp;
  | Imp -> coq_sterm c (Lam(mk_prop,Lam(mk_prop,Ap(Ap(Imp,DB(1,mk_prop)),DB(0,mk_prop))))) bound lp rp; 
  | Ap(Forall(a),Lam(_,m1)) -> (* forall with Lam *)
      begin
  ...
  | Forall(a) -> coq_sterm c (Lam(mk_ar a mk_prop,Ap(Forall(a),Lam(a,Ap(DB(1,mk_ar a mk_prop),DB(0,a)))))) bound lp rp
  | Ap(Ap(Eq(a1),m1),m2) when is_base a1 -> (* Equality *)
      if ((lp < 40) && (rp < 40)) then
	begin
...
let rec coq_spfterm c r hyp const bound =
 ...
 | Rewrite(prefix,pt,pt',r1) ->
   ...
   begin
     ...
     | Lam(ab,Lam(_,Ap(DB(1,_),DB(0,_)))) when is_ar ab ->
        (*let a = ty_get_l ab in*)
        (*let b = ty_get_r ab in*)
        (*** Skip etas - Mar 2012 ***)
...
***)
 ...
 | NegEqualFunc(((Ap(Ap(Imp,Ap(Ap(Eq(ab),s1),s2)),False)) as h),s,r1) when is_ar ab ->
    let a = ty_get_l ab in
    let b = ty_get_r ab in
     let h1 = get_hyp_name() in
     ...
     coq_spfterm c r1 ((coqnorm s,h1)::hyp) const bound;
     Printf.fprintf c ")";
 | EqualFunc(((Ap(Ap(Eq(ab),s1),s2)) as h),s,r1) when is_ar ab ->
    let a = ty_get_l ab in
    let b = ty_get_r ab in
     let h1 = get_hyp_name() in
 ...
 | Mating(h1,h2, ss, rs) ->
     ...
     begin
	   ...
	   coq_spfterm_dec c h3 (mk_ar a mk_prop) h2q h1p sr rr ((coqnorm (Ap(Ap(Imp,Ap(Ap(Eq(mk_ar a mk_prop),h1p),h2q)),False)),h3)::hyp) const bound; (** fix **)
	   Printf.fprintf c ") (fun %s => " h3;
	   ...
	   coq_sterm c h2q bound 5001 5000;
	   Printf.fprintf c " in ";
	   coq_spfterm_dec c h3 (mk_ar a mk_prop) h2q h1p sr rr ((coqnorm (Ap(Ap(Imp,Ap(Ap(Eq(mk_ar a mk_prop),h1p),h2q)),False)),h3)::(coqnorm (Ap(Ap(Imp,Ap(Ap(Eq(mk_ar a mk_prop),h2q),h1p)),False)),h4)::hyp) const bound;
	   Printf.fprintf c ") (fun %s => " h3;
	   coq_spfterm c r1 ((coqnorm s1,h3)::hyp) const bound;
	   Printf.fprintf c ")";
	   ...
	   coq_spfterm_dec c h3 (mk_ar a mk_prop) h1p h2q sr rr ((coqnorm (Ap(Ap(Imp,Ap(Ap(Eq(mk_ar a mk_prop),h1p),h2q)),False)),h3)::hyp) const bound;
	   Printf.fprintf c ") (fun %s => " h3;
	   coq_spfterm c r1 ((coqnorm s1,h3)::hyp) const bound;
	   Printf.fprintf c ")";
 ...
 | Confront(h1,h2,su,tu,sv,tv,r1,r2) ->
     ...
     begin
	   ...
	   begin
	     match h1c with
	     | (Ap(Ap(Eq(_),_),_)) ->
		 Printf.fprintf c " %s %s (fun %s %s => " (lookup "81" h1c hyp) (lookup "82" h2c hyp) h3 h4
	     | _ ->
		 Printf.fprintf c " %s %s (fun %s %s => " (lookup "81" h2c hyp) (lookup "82" h1c hyp) h3 h4
	   end;
  ...
  if ((coqnorm tl) = (coqnorm tr)) then
	...
	((((Ap(Ap(Imp,Ap(Ap(Eq(a),s11),s12)),False)) as s1)::sr),(r1::rr),Ap(tl1,_),Ap(tr1,_)) ->
	  ...
	  begin
	    ...
	    coq_spfterm_dec c h2 (mk_ar a b) tl1 tr1 sr rr ((coqnorm (Ap(Ap(Imp,Ap(Ap(Eq(mk_ar a b),tl1),tr1)),False)),h2)::hyp) const bound;
...
(** Prints refutation r to out_channel c **)
let ref_coq_spfterm c r = () (** deleted from Lash for now **)
...
let rec lookup_tstp_2016 w s hyp =
  try
    let (_,r) = List.find (fun ((_,x),_) -> x = s) hyp in r
  with
  | Not_found ->
      try
	...
	begin
	  ...
	  | Not_found ->
	      Printf.printf "%s: Could not find hyp name\ns = %s\nhyp:\n" w (trm_str s);
	      List.iter (fun ((n,m),h) -> Printf.printf "[%d] %s: %s\n" n h (trm_str m)) hyp;
	      failwith ("Could not find hyp name")
	end

let rec lookup_tstp_2016_f w sf s hyp =
  try
    let (_,r) = List.find (fun ((x,_),_) -> x = sf) hyp in r
  with
  | Not_found ->
     try
       let (x,m,c,f) = Hashtbl.find tstp_axioms_f sf in
       let rl = ref x in
       if (!f) then
	 begin
	   f := false;
       ...
       !rl
     with Not_found ->
       lookup_tstp_2016 (Printf.sprintf "%s:%d" w sf) s hyp

let rec lookup_tstp_2016_f_only w sf hyp =
  try
    let (_,r) = List.find (fun ((x,_),_) -> x = sf) hyp in r
  with
  | Not_found ->
...
let rec lookup_tstp_assumption_lit_2016 w c l hyp =
  try
    ...
    begin
      try
        let (_,r) = List.find (fun ((x,_),_) -> x = m1) hyp in r
      with Not_found ->
        let m1a = ftm_trm name_tp [] m1 in
	try
	  Hashtbl.find tstp_axiom_variants m1a
	with Not_found ->
          let m2a = ftm_trm name_tp [] m2 in
	  let p = lookup_tstp_2016_f w m2 m2a hyp in
	  if m1 = m2 then
	    p
	  else
	    begin
	      ...
	      incr tstpline;
	      Printf.fprintf c "thf(%d,plain," l;
	      trm_to_tstp_rembvar ll c m1a (Variables.make ());
	      Printf.fprintf c ",inference(normalize,[status(thm)],[%s]).\n" p;
	      Hashtbl.add tstp_axiom_variants m1a ll;
	      ll
	    end
    end
  with Not_found ->
...
let refut_tstp_clause c cr eigenchoicetps hyp clause =
  ...
  | ChoiceRule(m,pred) when get_tag m = FT_Choice -> ordinarycase "choice_rule"
  ...
  | ChoiceRule(m,pred) when get_tag m = FT_Name ->
      begin
        try
          let x = get_no m in
          let (a,mb) = Hashtbl.find choiceopnames_f x in
          begin
            try
              let (_,p1) = List.find (fun ((x,_),_) -> x = mb) hyp in
              let l = !tstpline in
              incr tstpline;
              Printf.fprintf c "thf(%d,plain,%s,inference(choice_rule,[status(thm)],[%s])).\n" l cs p1;
              string_of_int l
            with Not_found ->
              let p1 = lookup_tstp_2016_f_only "98" mb hyp in
              let l = !tstpline in
              incr tstpline;
              let p2 = string_of_int l in
              let mc = ftm_trm name_tp [] mb in
              Printf.fprintf c "thf(%d,plain," l;
              trm_to_tstp_rembvar p2 c mc (Variables.make ());
              Printf.fprintf c ",inference(preprocess,[status(thm)],[%s]).\n" p2;
              Hashtbl.add tstp_axiom_variants mc p2;
              let l = !tstpline in
              incr tstpline;
              Printf.fprintf c "thf(%d,plain,%s,inference(choice_rule,[status(thm)],[%s])).\n" l cs p2;
              string_of_int l
        ...
        with Not_found ->
          raise (Failure (Printf.sprintf "%s is used as a choice operator, but no corresponding proposition was recorded" (ftm_str m)))
      end
  | ChoiceRule(_,_) ->
     raise (Failure ("Bad term used as choice operator"))

let conshyp mf m h hyp =
  try
    let nf = Hashtbl.find preprocess_ftm mf in
    ((nf,m),h)::((mf,m),h)::hyp
  with Not_found -> ((mf,m),h)::hyp
...
let refut_tstp c r =
  ...
  Hashtbl.iter
    (fun s mf ->
      let m = ftm_trm name_tp [] mf in
      let fr = ref true in
      Hashtbl.add tstp_axioms m (s,m,c,fr);
      Hashtbl.add tstp_axioms_f mf (s,m,c,fr))
    name_hyp_f;
  let {basetypes;consttypes;eigendefs;propreferenced} = refutsiginfo_init () in
  refutsiginfo_of_refut name_tp_f {basetypes;consttypes;eigendefs;propreferenced} r;
  Hashtbl.remove basetypes tyname_dollar_i; (* TPTP already declares this *)
  Hashtbl.iter
    (fun no _ ->
      let basetype_name = no_basename no in
  ...
  Hashtbl.iter
    (fun name stp ->
      let tstp_name = tstpizename (name_no_rev name) in
      ...
      output_string c (").\n"))
    consttypes;
  let eigenchoicetps : (int * (string * string)) list ref = ref [] in
  Hashtbl.iter
    (fun name def ->
      ...
      | Ap(Choice(a),m) ->
	  let eigendef e =
	    tstp_print_eigendef c tstp_name (Ap(Name(e,mk_ar (mk_ar a mk_prop) a),m))
	  in
	  begin
	    try
	      let (e,_) = List.assoc (ty_f a) !eigenchoicetps in
	      eigendef e
	    with Not_found ->
	      begin
		let e = "eps__" ^ (string_of_int (List.length !eigenchoicetps)) in
		let s = forall (mk_ar a mk_prop) (forall a (imp (Ap(DB(1,mk_ar a mk_prop),DB(0,a))) (Ap(DB(1,mk_ar a mk_prop),Ap(Name(e,mk_ar (mk_ar a mk_prop) a),DB(1,mk_ar a mk_prop)))))) in
                let sf = mk_all (mk_ar a mk_prop) (mk_all a (mk_norm_imp (mk_norm_ap (mk_db 1) (mk_db 0)) (mk_norm_ap (mk_db 1) (mk_norm_ap (mk_name (name_no e)) (mk_db 1))))) in
		let h = get_thyp_name() in
		eigenchoicetps := (ty_f a,(e,h))::!eigenchoicetps;
  ...
  Hashtbl.iter
    (fun a _ ->
      let s = Atom.atom_to_trm a in
      tstp_print_defprop c !usednames (ftm_trm name_tp [] s)
      )
    propreferenced;
  List.iter
    (fun z ->
      match z with
      | ProbDef(x,tp,tm1,al,w) ->
	  Printf.fprintf c "thf(def_%s,definition,(%s = " x x;
	  trm_to_tstp_rembvar x c tm1 (Variables.make());
	  Printf.fprintf c ")).\n";
      | _ -> ())
    !probsig;
...
(**    Printf.printf "** refut_tstp_r\nr:\n"; print_refut r; Printf.printf "hyp:\n"; List.iter (fun (m,h) -> Printf.printf "%s: %s\n" h (trm_str m)) hyp; **)
	...
	let l1 = refut_tstp_r (conshyp nt1f ns1 h1 hyp) const r1 in
	let l2 = refut_tstp_r (conshyp t2f s2 h2 hyp) const r2 in
	let p1 = lookup_tstp_2016_f "6" t12f (Ap(Ap(Imp,s1),s2)) hyp in
	let l = !tstpline in
	incr tstpline;
	Printf.fprintf c "thf(%d,plain,$false,inference(tab_imp,[status(thm)%s],[%s,%d,%d%s])).\n" l (info_str "tab_imp" hyp [[h1];[h2]]) p1 l1 l2 (disch_str2 [h1;h2]);
	l
    | EqOR(t12f,t1f,t2f,t1,t2,r1,r2) ->
	let s1 = coqnorm t1 in
	let ns1 = normneg s1 in
	let s2 = coqnorm t2 in
	...
	let l1 = refut_tstp_r (conshyp t1f s1 h1 (conshyp t2f s2 h2 hyp)) const r1 in
	let l2 = refut_tstp_r (conshyp (fneg t1f) ns1 h3 (conshyp (fneg t2f) ns2 h4 hyp)) const r2 in
	let p1 = lookup_tstp_2016_f "23" t12f (Ap(Ap(Eq(mk_prop),s1),s2)) hyp in
	let l = !tstpline in
	incr tstpline;
	Printf.fprintf c "thf(%d,plain,$false,inference(tab_bq,[status(thm)%s],[%s,%d,%d%s])).\n" l (info_str "tab_bq" hyp [[h1;h2];[h3;h4]]) p1 l1 l2 (disch_str2 [h1;h2;h3;h4]);
	l
    | NegEqOR(tf,t1f,t2f,t1,t2,r1,r2) ->
	let s1 = coqnorm t1 in
	let ns1 = normneg s1 in
	let s2 = coqnorm t2 in
	let ns2 = normneg s2 in
	...
	let h3 = get_tstp_hyp_name c ns1 in
	let h4 = get_tstp_hyp_name c ns2 in
	let l1 = refut_tstp_r (conshyp t1f s1 h1 (conshyp (fneg t2f) ns2 h2 hyp)) const r1 in
	let l2 = refut_tstp_r (conshyp (fneg t1f) ns1 h3 (conshyp t2f s2 h4 hyp)) const r2 in
	let p1 = lookup_tstp_2016_f "22" tf (neg(Ap(Ap(Eq(mk_prop),s1),s2))) hyp in
	let l = !tstpline in
	incr tstpline;
	Printf.fprintf c "thf(%d,plain,$false,inference(tab_be,[status(thm)%s],[%s,%d,%d%s])).\n" l (info_str "tab_be" hyp [[h1;h2];[h3;h4]]) p1 l1 l2 (disch_str2 [h1;h2;h3;h4]);
	l
    | NegAllR(a,tf,nt1f,t,x,r) ->
	let s1 = coqnorm (norm name_def (ap(t,Name(x,a)))) in
	let ns1 = normneg s1 in
	let h1 = get_tstp_hyp_name c ns1 in
	let l1 = refut_tstp_r (conshyp nt1f ns1 h1 hyp) ((x,a)::const) r in
	let p1 = lookup_tstp_2016_f "11" tf (neg(Ap(Forall(a),t))) hyp in
	let l = !tstpline in
	incr tstpline;
	Printf.fprintf c "thf(%d,plain,$false,inference(tab_negall,[status(thm)%s,tab_negall(eigenvar,%s)],[%s,%d%s])).\n" l (info_str "tab_negall" hyp [[h1]]) (tstpizename x) p1 l1 (disch_str2 [h1]);
	l
    | NegEqFR(a,b,tf,ntfq,t1,t2,r) ->
	let t = coqnorm (forall a (eq b (ap(t1,DB(0,a))) (ap(t2,DB(0,a))))) in
	let nt = neg t in
	let s = coqnorm (eq (mk_ar a b) t1 t2) in
	let ns = neg s in
	let h1 = get_tstp_hyp_name c nt in
	let l1 = refut_tstp_r (conshyp ntfq nt h1 hyp) const r in
	let p1 = lookup_tstp_2016_f "25" tf ns hyp in
	let l = !tstpline in
	incr tstpline;
	Printf.fprintf c "thf(%d,plain,$false,inference(tab_fe,[status(thm)%s],[%s,%d%s])).\n" l (info_str "tab_fe" hyp [[h1]]) p1 l1 (disch_str2 [h1]);
	l
    | NegImpR(tf,t1f,nt2f,t1,t2,r) ->
	let s1 = coqnorm t1 in
	let s2 = coqnorm t2 in
	let ns2 = normneg s2 in
	let h1 = get_tstp_hyp_name c s1 in
	let h2 = get_tstp_hyp_name c ns2 in
	let l1 = refut_tstp_r (conshyp t1f s1 h1 (conshyp nt2f ns2 h2 hyp)) const r in
	let p1 = lookup_tstp_2016_f "7" tf (normneg(Ap(Ap(Imp,s1),s2))) hyp in
	let l = !tstpline in
	incr tstpline;
	Printf.fprintf c "thf(%d,plain,$false,inference(tab_negimp,[status(thm)%s],[%s,%d%s])).\n" l (info_str "tab_negimp" hyp [[h1;h2]]) p1 l1 (disch_str2 [h1;h2]);
	l
  ...
  | [(con,nncon)] ->
      let ccon = ftm_trm name_tp [] con in
      let ncon = ftm_trm name_tp [] nncon in
      let ccon = coqnorm ccon in (*conjecture*)
      let ncon = coqnorm ncon in (*negated conjecture*)
      let h1 = get_thyp_name() in (*hypothesis name*)
      Printf.fprintf c "thf(%s,conjecture," (match !conjecturenames with [] -> "claim" | cname::_ -> cname);
      flush c;
      trm_to_tstp c ccon (Variables.make ());
      flush c;
      Printf.fprintf c ").\n";
      ...
      trm_to_tstp_rembvar h1 c ncon (Variables.make ());
      flush c;
      Printf.fprintf c ",inference(assume_negation,[status(cth)],[%s])).\n" (match !conjecturenames with [] -> "claim" | cname::_ -> cname);
      flush c;
      let l = refut_tstp_r (conshyp nncon ncon h1 !prehyp) [] r in
      discharge_eigenhyps_c l !eigenhyp ((nncon,ncon),h1) !prehyp;
      Printf.fprintf c "thf(0,theorem,";
      flush c;
      trm_to_tstp c ccon (Variables.make ()); (*print the conjecture again*)
      flush c;
