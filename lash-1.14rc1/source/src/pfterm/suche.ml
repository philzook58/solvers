(* Lash *)
(* ported from Satallax file: pfterm/such.ml *)
...
(** Input: List of clauses cl,cw and clause to rule info cr
		...
		| ConfrontationRule(plit,nlit) ->
			...
			let c2 = match (n,m) with
				| (  Ap(Ap(Eq(a),s),t)  ,  Ap(Ap(Eq(a'),u),v)  ) when a=a' -> begin
					...
					let (lsu,ltu) = (loc_get_literal su,loc_get_literal tu) in 
					[-nlit;-plit;lsu;ltu]					 
					end
...
let get_next_step b sl  = 
	...
	try gnc [] b sl with No_More_Clauses ->
          (match sl with x::_ -> x | [] -> failwith "No clauses left to choose from")
...
and apply_Imp  b h s t cl = begin 
		...
		(Implication(loc_literal_to_trm h,loc_literal_to_trm s,loc_literal_to_trm t, r1, r2),con) 
		else 
		let level = Branch.next_level b in
		let (r,c) = try or_search [t;-s] b cl  
			with Independent_Refutation(r,con)-> (r,con) in
		if pftrm_Timeout && (!isTimeout) then (Implication(loc_literal_to_trm h,loc_literal_to_trm s,loc_literal_to_trm t,Timeout,r),Dependency.empty)
		else
		...
		then
			...
			then (Implication(loc_literal_to_trm h,loc_literal_to_trm s,loc_literal_to_trm t,r',r),Dependency.empty)
			else
			...
			then 	(* semantic branching with cut on s *)
				...
				(Cut(loc_literal_to_trm s,r',Implication(loc_literal_to_trm h,loc_literal_to_trm s,loc_literal_to_trm t,
								myConflict 1 (loc_literal_to_trm s,loc_literal_to_trm (-s)),r)),con) 
		...
		then 
			...
			then 	(* only cut on s*)
				...
				(Cut(loc_literal_to_trm s,r',r),con)
			else   (* Second subrefutation is independent *)
		...
		then 
			...
			then (* semantic branching with cut on t *)
				...
				(Cut(loc_literal_to_trm t,r,Implication(loc_literal_to_trm h,loc_literal_to_trm s,loc_literal_to_trm t,r',
						myConflict 2 (loc_literal_to_trm t,loc_literal_to_trm (-t)))),con) 					
			else 
			...
			then  (* syntactic branching *)
				...
				(Implication(loc_literal_to_trm h,loc_literal_to_trm s,loc_literal_to_trm t,r',r),con)
			else 
			...
			then (* only cut on t*)
				...
				if debug_Independent_refutation then Printf.printf "condition(5) = %s\n" (String.concat "," (List.map string_of_int (Dependency.elements con)));
					(Cut(loc_literal_to_trm t,r,r'),con) 
			else
...
and apply_Con b cl br h =
  begin
    ...
    let (ss,tt) = match loc_literal_to_trm (-st) with |  Ap(Ap(Eq(a),s),t) -> (Ap(Ap(Imp,Ap(Ap(Eq(a),s),s) ),False),Ap(Ap(Imp,Ap(Ap(Eq(a),t),t) ),False)) | _ -> failwith "error apply_con" in
    ...
    if Branch.mem (-su) b || Branch.mem (-tu) b || Branch.mem (-sv) b || Branch.mem (-tv) b then 
      ...
      (Confront(loc_literal_to_trm (-st),loc_literal_to_trm uv,loc_literal_to_trm su,loc_literal_to_trm tu,loc_literal_to_trm sv,loc_literal_to_trm tv,r1,r2),con)
      ...
      if pftrm_Timeout && (!isTimeout) 
      then (Confront(loc_literal_to_trm (-st),loc_literal_to_trm uv,loc_literal_to_trm su,loc_literal_to_trm tu,loc_literal_to_trm sv,loc_literal_to_trm tv, r, Timeout),Dependency.empty) 
      else 
	let [b1;b2] = Dependency.check d [tv;sv] in
	if b1 || b2 then 
	  let r1 = myConflict 3 (loc_literal_to_trm (-st),loc_literal_to_trm st) in
	  ...
	  if svb then ( if debug_Independent_refutation then Printf.printf  "apply_Con 3\n";
				   (Confront(loc_literal_to_trm (-st),loc_literal_to_trm sv,ss,loc_literal_to_trm st,loc_literal_to_trm sv,loc_literal_to_trm tv,r1,r),
		...
		(Confront(loc_literal_to_trm (-st),loc_literal_to_trm tv,loc_literal_to_trm st,tt,loc_literal_to_trm sv,loc_literal_to_trm tv,r1,r),
		 Dependency.diffunion
	...
	if b1 || b2 then 
	  let r1 = myConflict 4 (loc_literal_to_trm (-st),loc_literal_to_trm st) in
	  ...
	  if sub then ( if debug_Independent_refutation then Printf.printf  "apply_Con 5\n";
				   (Confront(loc_literal_to_trm (-st),loc_literal_to_trm su,ss,loc_literal_to_trm st,loc_literal_to_trm su,loc_literal_to_trm tu,r1,r),
				    Dependency.diffunion [-st;su] [d] [[tu;su]]) )
	  else ( if debug_Independent_refutation then Printf.printf  "apply_Con 6\n";
		(Confront(loc_literal_to_trm (-st),loc_literal_to_trm tu,loc_literal_to_trm st,tt,loc_literal_to_trm su,loc_literal_to_trm tu,r1,r),
	...
	| (true,true) -> begin 
	    ...
	    if pftrm_Timeout && (!isTimeout) 
	    then (Confront(loc_literal_to_trm (-st),loc_literal_to_trm uv,loc_literal_to_trm su,loc_literal_to_trm tu,loc_literal_to_trm sv,loc_literal_to_trm tv, r, r'),Dependency.empty) 
	    else
	      begin match Dependency.check d' [tv;sv] with
	      | [true;true] -> begin 
		  let r1 = myConflict 5 (loc_literal_to_trm (-st),loc_literal_to_trm st) in
		  if b1 then
		    let r2 = Confront(loc_literal_to_trm (-st),loc_literal_to_trm tv,loc_literal_to_trm st,tt,loc_literal_to_trm sv,loc_literal_to_trm tv, r1, r') in
		    let r3 = myConflict 6 (loc_literal_to_trm (-tv),loc_literal_to_trm tv)  in
		    let r4 = Confront(loc_literal_to_trm (-st),loc_literal_to_trm uv,loc_literal_to_trm su,loc_literal_to_trm tu,loc_literal_to_trm sv,loc_literal_to_trm tv,r,r3) in
		    let r5 = if b2 then Trans(loc_literal_to_trm (-st),loc_literal_to_trm (-tv),loc_literal_to_trm (-sv),r4) else r4 in
		    ( if debug_Independent_refutation then Printf.printf  "apply_Con 9\n";(Cut(loc_literal_to_trm (-tv),r5,r2),Dependency.diffunion h [d;d'] [[-tv;-sv;su;tu];[tv;sv]])) 	
		  else 
		    let r2 = Confront(loc_literal_to_trm (-st),loc_literal_to_trm sv,ss,loc_literal_to_trm st,loc_literal_to_trm sv,loc_literal_to_trm tv, r1, r') in
		    let r3 = myConflict 7 (loc_literal_to_trm (-sv),loc_literal_to_trm sv) in
		    let r4 = Confront(loc_literal_to_trm (-st),loc_literal_to_trm uv,loc_literal_to_trm su,loc_literal_to_trm tu,loc_literal_to_trm sv,loc_literal_to_trm tv,r,r3) in
		    (if debug_Independent_refutation then Printf.printf  "apply_Con 10\n";(Cut(loc_literal_to_trm (-sv),r4,r2),Dependency.diffunion h [d;d'] [[-tv;-sv;su;tu];[tv;sv]])) 				
	        end
	      | [true;false] -> begin 
		  let r1 = myConflict 8 (loc_literal_to_trm (-tv),loc_literal_to_trm tv)  in
		  let r2 = Confront(loc_literal_to_trm (-st),loc_literal_to_trm uv,loc_literal_to_trm su,loc_literal_to_trm tu,loc_literal_to_trm sv,loc_literal_to_trm tv,r,r1) in
		  let r3 = if b2 then Trans(loc_literal_to_trm (-st),loc_literal_to_trm (-tv),loc_literal_to_trm (-sv),r2) else r2 in
		  (if debug_Independent_refutation then Printf.printf  "apply_Con 11\n";(Cut(loc_literal_to_trm (-tv),r3,r'),Dependency.diffunion h [d;d'] [[-tv;-sv;su;tu];[tv;sv]])) 				
	        end
	      | [false;true] -> begin 
		  let r1 = myConflict 9 (loc_literal_to_trm (-sv),loc_literal_to_trm sv) in
		  let r2 = Confront(loc_literal_to_trm (-st),loc_literal_to_trm uv,loc_literal_to_trm su,loc_literal_to_trm tu,loc_literal_to_trm sv,loc_literal_to_trm tv,r,r1) in
		  let r3 = if b1 then Trans(loc_literal_to_trm (-st),loc_literal_to_trm (-sv),loc_literal_to_trm (-tv),r2) else r2 in
		  (if debug_Independent_refutation then Printf.printf  "apply_Con 12\n";(Cut(loc_literal_to_trm (-sv),r3,r'),Dependency.diffunion h [d;d'] [[-tv;-sv;su;tu];[tv;sv]]) )	
	        end
	      | [false;false] -> begin
	...
	| (true,false) -> begin 
	    ...
	    then (Confront(loc_literal_to_trm (-st),loc_literal_to_trm uv,loc_literal_to_trm su,loc_literal_to_trm tu,loc_literal_to_trm sv,loc_literal_to_trm tv, r, r'),Dependency.empty) 	
	      ...
	      match (b5,b6) with
	      | (true,true) -> begin 
		  let r1 = myConflict 10 (loc_literal_to_trm (-st),loc_literal_to_trm st) in
		  if b1 then
		    let r2 = Confront(loc_literal_to_trm (-st),loc_literal_to_trm tv,loc_literal_to_trm st,tt,loc_literal_to_trm sv,loc_literal_to_trm tv, r1, r') in
		    let r3 = if b2 then Trans(loc_literal_to_trm (-st),loc_literal_to_trm (-tv),loc_literal_to_trm (-sv),r) else r in
		    (if debug_Independent_refutation then Printf.printf  "apply_Con 13\n";(Cut(loc_literal_to_trm (-tv),r3,r2),Dependency.diffunion h [d;d'] [[-tv;-sv;su;tu];[tv;sv]]) 	)
		  else 
		    let r2 = Confront(loc_literal_to_trm (-st),loc_literal_to_trm sv,ss,loc_literal_to_trm st,loc_literal_to_trm sv,loc_literal_to_trm tv, r1, r') in
		    (if debug_Independent_refutation then  Printf.printf  "apply_Con 14\n";(Cut(loc_literal_to_trm (-sv),r,r2),Dependency.diffunion h [d;d'] [[-tv;-sv;su;tu];[tv;sv]]) ) 				
	      end
	      | (true,false) -> begin 
		  let r1 = if b2 then Trans(loc_literal_to_trm (-st),loc_literal_to_trm (-tv),loc_literal_to_trm (-sv),r) else r in
		  (if debug_Independent_refutation then  Printf.printf  "apply_Con 15\n";(Cut(loc_literal_to_trm (-tv),r1,r'),Dependency.diffunion h [d;d'] [[-tv;-sv;su;tu];[tv;sv]]) ) 				
	      end
	      | (false,true) -> begin 
		  let r1 = if b1 then Trans(loc_literal_to_trm (-st),loc_literal_to_trm (-sv),loc_literal_to_trm (-tv),r) else r in
		  (if debug_Independent_refutation then  Printf.printf  "apply_Con 16\n";(Cut(loc_literal_to_trm (-sv),r1,r'),Dependency.diffunion h [d;d'] [[-tv;-sv;su;tu];[tv;sv]]) )	
	      end
	      | (false,false) -> begin 
		  (if debug_Independent_refutation then Printf.printf  "apply_Con Independent_Refutation left(3)\n" ;
		   raise (Independent_Refutation(r',d') ) )				
	...
	| (false,true) -> begin 
	    ...
	    then (Confront(loc_literal_to_trm (-st),loc_literal_to_trm uv,loc_literal_to_trm su,loc_literal_to_trm tu,loc_literal_to_trm sv,loc_literal_to_trm tv, r, r'),Dependency.empty) 
	      ...
	      | (true,true) -> begin 
		  let r1 = myConflict 11 (loc_literal_to_trm (-st),loc_literal_to_trm st) in
		  if b3 then
		    let r2 = if b4 then 
		      Confront(loc_literal_to_trm (-st),loc_literal_to_trm tu,loc_literal_to_trm st,tt,loc_literal_to_trm su,loc_literal_to_trm tu, r1, r) 
		    else r in
		    let r3 = myConflict 12 (loc_literal_to_trm (-tu),loc_literal_to_trm tu)  in
		    let r4 = Confront(loc_literal_to_trm (-st),loc_literal_to_trm tu,loc_literal_to_trm su,loc_literal_to_trm tu,loc_literal_to_trm sv,loc_literal_to_trm tv,r3,r') in
		    let r5 = if b2 then Trans(loc_literal_to_trm (-st),loc_literal_to_trm (-tu),loc_literal_to_trm (-su),r4) else r4 in
		    (if debug_Independent_refutation then  Printf.printf  "apply_Con 17\n";(Cut(loc_literal_to_trm (-tu),r5,r2),Dependency.diffunion h [d;d'] [[-tv;-sv;su;tu];[tv;sv]]) )	
		  else 
		    let r2 = r in
		    let r3 = myConflict 13 (loc_literal_to_trm (-su),loc_literal_to_trm su) in
		    let r4 = Confront(loc_literal_to_trm (-st),loc_literal_to_trm uv,loc_literal_to_trm su,loc_literal_to_trm tu,loc_literal_to_trm sv,loc_literal_to_trm tv,r3,r') in
		    let r5 = if b1 then Trans(loc_literal_to_trm (-st),loc_literal_to_trm (-su),loc_literal_to_trm (-tu),r4) else r4 in
		    ( if debug_Independent_refutation then Printf.printf  "apply_Con 18\n";(Cut(loc_literal_to_trm (-su),r5,r2),Dependency.diffunion h [d;d'] [[-tv;-sv;su;tu];[tv;sv]])	)		
	      end
	      | (true,false) -> begin 
		  if b3 then
		    let r1 = myConflict 14 (loc_literal_to_trm (-st),loc_literal_to_trm st) in
		    let r2 = if b4 then Confront(loc_literal_to_trm (-st),loc_literal_to_trm tu,loc_literal_to_trm st,tt,loc_literal_to_trm su,loc_literal_to_trm tu, r1, r) else r in
		    let r3 = if b2 then Trans(loc_literal_to_trm (-st),loc_literal_to_trm (-tu),loc_literal_to_trm (-su),r') else r' in
		    if debug_Independent_refutation then Printf.printf  "apply_Con 19\n";(Cut(loc_literal_to_trm (-tu),r3,r2),Dependency.diffunion h [d;d'] [[su;tu];[-tu;-su;tv;sv]])
		  else
		    let r1 = if b1 then Trans(loc_literal_to_trm (-st),loc_literal_to_trm (-su),loc_literal_to_trm (-tu),r') else r' in
		    if debug_Independent_refutation then  Printf.printf  "apply_Con 20\n";(Cut(loc_literal_to_trm (-su),r1,r),Dependency.diffunion h [d;d'] [[su;tu];[-tu;-su;tv;sv]])
	      end
	      | (false,true) -> begin 
		  (if debug_Independent_refutation then  Printf.printf  "apply_Con 21\n";(Confront(loc_literal_to_trm(-st),loc_literal_to_trm uv,loc_literal_to_trm su,loc_literal_to_trm tu,loc_literal_to_trm sv,loc_literal_to_trm tv,r,r'),Dependency.diffunion h [d;d'] [[su;tu];[tv;sv]]) )
	      end
	      | (false,false) -> begin 
...
and apply_mat b sl alts pf mdf = 
	...
	let (r,dep,dg) = 
		...
		| None -> (* normal case: construct the mating/decomposition step at the base of the semantic branching step *)
			...
			let rds = List.map (fun a-> 
				...
				(myConflict 15 (loc_literal_to_trm l,loc_literal_to_trm (-l)),Dependency.make [l;-l]) ) alts in
			let (rs,ds) = List.split rds in
			let dep = Dependency.diffunion pf ds alts in
			(* mdf = true -> mating | false -> decomposition *)
			if mdf then (Mating(loc_literal_to_trm  (List.nth pf 0),loc_literal_to_trm  (List.nth pf 1),
					List.map (fun alt -> loc_literal_to_trm (List.hd alt) ) alts,rs),dep,dg)
			else (Decomposition(loc_literal_to_trm  (List.nth pf 0),
					List.map (fun alt -> loc_literal_to_trm (List.hd alt) ) alts,rs),dep,dg)		
			end
		in
	...
	List.fold_left 
		(fun (r1,d1) l -> 
			...
			let r' = Cut(loc_literal_to_trm l,r1,r2) in
...
and and_search b c sl  =
	...
	(Cut( loc_literal_to_trm (abs cutlit) , r2, r1),con)
	...
	| Step.IMP -> 
	   ...
	   | [[s];[t]] ->
	      ...
	      then	apply_Imp b head s t sl
                ...
                begin match apply_rule b sl br h with
                | (r1::r2::rs,con) ->
            	   Implication(loc_literal_to_trm head,loc_literal_to_trm s,loc_literal_to_trm t, r1, r2),con
                | _ -> assert false
                end
           | _ -> assert false
           end
	| Step.CON ->
	   let h1 =loc_literal_to_trm (List.hd h) in
           let h2 =loc_literal_to_trm (List.hd (List.tl h)) in
           begin match br with
           | [[su;tu];[sv;tv]] ->
		...
		apply_Con b sl br h
 		...
 		begin match apply_rule b sl br h with
                | (r1::r2::rs,con) ->
		   (Confront(h1,h2,loc_literal_to_trm su,loc_literal_to_trm tu,loc_literal_to_trm sv,loc_literal_to_trm tv, r1, r2),con)
                | _ -> assert false
                end
           | _ -> assert false
           end
	| Step.MAT ->
		...
		let (r,con) = apply_rule b sl br h  in
            	let  ss =  List.map (fun ls -> loc_literal_to_trm (List.hd ls) ) br  in
		begin match h with
                | [h1;h2] ->
	    	   (Mating(loc_literal_to_trm h1,loc_literal_to_trm h2,ss , r),con)       
                | _ -> assert false
                end
	| Step.DEC ->  
		if (get_bool_flag "PFTRM_REMOVE_INDEPENDENT") && pftrm_Independent_refutation && pftrm_semantic_mat_dec	
		...
		let (r,con) = apply_rule b sl br h  in
		let h = loc_literal_to_trm (List.hd h) in
		let ss = List.map (fun ls -> loc_literal_to_trm (List.hd ls) ) br in	
	...
	| Step.NIMP -> 
	   ...
	   begin match br, r with
           | [[s;t]], r1::rs ->
              (NegImplication(h,loc_literal_to_trm s,loc_literal_to_trm t,r1),con)
           | _ -> assert false
           end
	| Step.ALL(a,m,n) ->
	   let h = loc_literal_to_trm (List.hd h) in
           begin match br, r with
	   | [[s]], r1::rs -> (All(h,loc_literal_to_trm s,r1,a,m,n),con) 
           | _ -> assert false
           end
	| Step.NALL(a,m,x) -> 
	   if assert_freshness && List.exists (fun t -> List.mem x (Step.trm_free_variable (loc_literal_to_trm t)))(Branch.elements b ) 
	   then failwith "found freshness violation "  ; 
	   let head = loc_literal_to_trm (List.hd h) in
           begin match br, r with
	   | [[s]], r1::rs ->
              (NegAll(head,loc_literal_to_trm s,r1,a,m,x),con)  
           | _ -> assert false
           end
	| Step.MAT ->  
	   begin match h with
	...
	| Step.BE ->
	   let h =loc_literal_to_trm (List.hd h) in
           begin match br, r with
           | [[s;_];[_;t]], r1::r2::rs ->
	      (NegEqualProp(h,loc_literal_to_trm s,loc_literal_to_trm t, r1, r2),con)
           | _ -> assert false
           end
	| Step.BQ ->
	   let h =loc_literal_to_trm (List.hd h) in
           begin match br, r with
	   | [[s;t];[_;_]], r1::r2::rs ->
	      (EqualProp(h,loc_literal_to_trm s,loc_literal_to_trm t, r1, r2),con)
           | _ -> assert false
           end
	| Step.FE -> 
	   let h = loc_literal_to_trm (List.hd h) in
	   let s = loc_literal_to_trm (List.hd (List.hd br)) in
           begin match r with
           | r1::rs -> NegEqualFunc(h, s, r1),con
           | _ -> assert false
           end
	| Step.FQ ->  
	   let h = loc_literal_to_trm (List.hd h) in
	   let s = loc_literal_to_trm (List.hd (List.hd br)) in
           begin match r with
           | r1::rs -> EqualFunc(h, s, r1),con
           | _ -> assert false
           end
	| Step.EPS(eps,pred) ->
           begin match br, r with
           | [[empty];[choice]], r1::r2::rs ->
	      (ChoiceR(eps,pred,loc_literal_to_trm empty,loc_literal_to_trm choice,r1, r2 ),con) 
           | _ -> assert false
           end
	| Step.CUT ->
           begin match br, r with
           | [_;[h]], r1::r2::rs -> Cut( loc_literal_to_trm h , r2, r1),con
           | _ -> assert false
           end
	| Step.KNOWN(s,name,al) ->
           begin match br, r with
           | [[kn]], r1::rs -> KnownResult(s,name,al,r1),con
           | _ -> assert false
           end
	end
	end          
...
and leibeq_to_primeq b sl =
	let rec leibeq_to_primeq1 b' b = match b' with
		...
		| (m::br) -> try
    			...
    			let (pre,pt,pt',stp)= leibeq_to_primeq2 (loc_literal_to_trm m) 0 in
    			let prefix= Lam(stp,pre) in
    			let ptrm' =loc_get_literal (norm name_def (Ap(prefix,pt'))) in
			begin
			try 
...
let rec process_refut1 b r =  
  ...
  | NegImpR(_,_,_,m,n,r1) -> if debug_search then Printf.printf  "apply NegImpR\n";
    	let h = (normneg (imp m n)) in
    	let s = m in let t = (normneg n) in
	let s' = loc_get_literal s in let t'= loc_get_literal t in
...
(**	if debug_search then Printf.printf "con dependencies after recursive call from NegImpR:\n%s\n" (List.fold_right (fun z r -> r ^ (string_of_int z) ^ "\n") (Dependency.elements con) ""); **)
	...
	let con = Dependency.diffunion [loc_get_literal h] [con] [[s';t']] in
    	(NegImplication(h,s,t,r),con)

  | ImpR(_,_,_,m,n,r1,r2) -> if debug_search then Printf.printf  "apply ImpR\n";
...
(**	if debug_search then Printf.printf "con dependencies after second recursive call from ImpR:\n%s\n" (List.fold_right (fun z r -> r ^ (string_of_int z) ^ "\n") (Dependency.elements con') ""); **)
	...
	let con = Dependency.diffunion [loc_get_literal h] [con;con'] [[s'];[t']]  in
   	(Implication(h,s,t,r ,r'),con)

  | FalseR ->  if debug_search then Printf.printf  "apply FalseR\n"; (Fal(False),Dependency.make [loc_get_literal False])

  | NegReflR(_,_) -> if debug_search then Printf.printf  "apply NegReflR \n";
	let h = List.find (fun t -> match (loc_literal_to_trm t) with 
                                     		| Ap(Ap(Imp,Ap(Ap(Eq(_),s),t)),False)->  s = t
                                     		| _ -> false ) (Branch.elements b) in
	(NegRefl(loc_literal_to_trm h),Dependency.make [h]) 

  | NegAllR(a,_,_,m,x,r1) -> if debug_search then Printf.printf  "apply NegAllR with branch %s\n" (List.fold_right (fun l s -> (string_of_int l) ^ " " ^ s) (Branch.elements b) "");
    	let h = (normneg (Ap(Forall(a),m))) in 
    	let s = (norm name_def (normneg (Ap(m,Name(x,a)))) ) in 
	let s' = loc_get_literal s in
	...
	let con = Dependency.diffunion [loc_get_literal h] [con] [[s']] in
    	(NegAll(h,s,r,a,m,x),con)

  | NegEqFR(a,a',_,_,s,t,r1) -> if debug_search then Printf.printf  "apply NegEqFR\n";
    	let h = neg (eq (mk_ar a a') s t) in
    	let m = normneg (norm name_def (forall a (eq a' (Ap(s,DB(0,a))) (Ap(t,DB(0,a))) ))) in
	let s' = loc_get_literal m in
	let (r,con)=try process_refut [s'] b r1 with Independent_Refutation(r,con)-> (r,con) in
	if (get_bool_flag "PFTRM_REMOVE_INDEPENDENT") && pftrm_Independent_refutation  && not (Dependency.mem s' con) then
		( if debug_Independent_refutation then Printf.printf  "found Independent refutation\n";
			raise (Independent_Refutation(r,con)) )
  ...
  | EqOR(_,_,_,s,t,r1,r2) -> if debug_search then Printf.printf  "apply EqOR\n";
	let l = Branch.next_level b in
    	let h =  (eq mk_prop s t) in
    	let ns =  (normneg s) in let nt =  (normneg t) in
	let s' = loc_get_literal s in let t'= loc_get_literal t in
	let ns' = loc_get_literal ns in let nt'= loc_get_literal nt in
	let (r,con)= try process_refut [s';t'] b r1 with Independent_Refutation(r,con)-> (r,con) in
	if (get_bool_flag "PFTRM_REMOVE_INDEPENDENT") && pftrm_Independent_refutation  && not (Dependency.mem s' con || Dependency.mem t' con) then
		( if debug_Independent_refutation then Printf.printf  "found Independent refutation\n";
			raise (Independent_Refutation(r,con)) )
	...
	let con = Dependency.diffunion [loc_get_literal h] [con;con'] [[s';t'];[ns';nt']] in
     	(EqualProp(h,s,t,r,r'),con)

  | NegEqOR(_,_,_,s,t,r1,r2) -> if debug_search then Printf.printf  "apply NegEqOR\n";
	let l = Branch.next_level b in
    	let h =  neg(eq mk_prop s t) in
    	let ns =  (normneg s) in let nt =  (normneg t) in
	let s' = loc_get_literal s in let t'= loc_get_literal t in
	let ns' = loc_get_literal ns in let nt'= loc_get_literal nt in
	...
	let con = Dependency.diffunion [loc_get_literal h] [con;con'] [[s';nt'];[ns';t']] in
     	(NegEqualProp(h,s,t,r,r'),con) 

  | AssumptionConflictR(_) -> 
		if (get_bool_flag "LEIBEQ_TO_PRIMEQ") then
	    	leibeq_to_primeq b []
