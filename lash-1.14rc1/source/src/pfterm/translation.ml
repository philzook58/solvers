(* Lash *)
(* ported from Satallax file: pfterm/translation.ml *)
...
let coqnorm n = n (** coqnorm does nothing in Lash **)
...
let rec tpof m =
  match m with
    Name(_,a) -> a
  | False -> mk_prop
  | Imp -> mk_ar mk_prop (mk_ar mk_prop mk_prop)
  | Forall(a) -> mk_ar (mk_ar a mk_prop) mk_prop
  | Eq(a) -> mk_ar a (mk_ar a mk_prop)
  | Choice(a) -> mk_ar (mk_ar a mk_prop) a
  | True -> mk_prop
  | And -> mk_ar mk_prop (mk_ar mk_prop mk_prop)
  | Or -> mk_ar mk_prop (mk_ar mk_prop mk_prop)
  | Iff -> mk_ar mk_prop (mk_ar mk_prop mk_prop)
  | Neg -> mk_ar mk_prop mk_prop
  | Exists(a) -> mk_ar (mk_ar a mk_prop) mk_prop
  | DB(_,a) -> a
...
let rec rewrite_refutation1 ptrm n =  
  ...
  | Name(x,a) -> (* delta *)
      begin
	try
	let def = onlynegnorm (Hashtbl.find State.name_def_prenorm x) in
  ...
  | Lam(a,Ap(p,DB(0,_))) when (termNotFreeIn p 0) -> (* eta *)
	let b= tpof p in
    	let prefix = Ap(DB(n,mk_ar b b),shift p 0 (- 1)) in
    	let ptrm = Lam(b,Lam(a,Ap(DB(1,b),DB(0,a)))) in
    	let ptrm' = Lam(b,DB(0,b)) in 
      (prefix,ptrm,ptrm',true,mk_ar b b)
  | Ap(Ap(Imp,Ap(Ap(Imp,m1),False)),False) -> (* double negation reduce *)
    let prefix = Ap(DB(n,mk_ar mk_prop mk_prop),m1) in
    let ptrm = Lam(mk_ar mk_prop mk_prop,Ap(Ap(Imp,Ap(Ap(Imp,DB(0,mk_prop)),False)),False)) in
    let ptrm' = Lam(mk_ar mk_prop mk_prop,DB(0,mk_prop)) in 
      (prefix,ptrm,ptrm',true,mk_ar mk_prop mk_prop)
  | True -> (DB(n,mk_prop),True,imp False False,true,mk_prop) (* True => False -> False *)
  | Neg -> (DB(n,mk_ar mk_prop mk_prop),Neg,Lam(mk_prop,imp (DB(0,mk_prop)) False),false,mk_ar mk_prop mk_prop) (* ~s => s -> False , b is false as this is only a notation for Coq*) 
  | Or ->  (DB(n,mk_ar mk_prop (mk_ar mk_prop mk_prop)),Or,Lam(mk_prop,Lam(mk_prop,disj (DB(1,mk_prop)) (DB(0,mk_prop)))),true,mk_ar mk_prop (mk_ar mk_prop mk_prop)) (* s \/ t => ~s -> t*)
  | And -> if  debug_rewrite then Printf.printf  "Rewrite And \n"; 
	(DB(n,mk_ar mk_prop (mk_ar mk_prop mk_prop)),And,Lam(mk_prop,Lam(mk_prop,conj (DB(1,mk_prop)) (DB(0,mk_prop)))),true,mk_ar mk_prop (mk_ar mk_prop mk_prop)) (* s /\ t => ~(s -> ~t)*)   
  | Iff -> (DB(n,mk_ar mk_prop (mk_ar mk_prop mk_prop)),Iff,Lam(mk_prop,Lam(mk_prop,iff (DB(1,mk_prop)) (DB(0,mk_prop)))),true,mk_ar mk_prop (mk_ar mk_prop mk_prop)) (* s <-> t => s = t*)
  | Exists(a) ->
	let ao = mk_ar a mk_prop in
	let aoo = mk_ar (mk_ar a mk_prop) mk_prop in
...
| Decomposition(h1,ss, rs) -> begin 
   ...
   try
    let pss = match p with  
     | (Ap(Ap(Imp,Ap(Ap(Eq(a1),m1),m2)),False)) when is_base a1 -> 
...
| NegEqualProp(h,s,t,r1,r2) -> begin
   ...
   match p with  
     | (Ap(Ap(Imp,Ap(Ap(Eq(mk_prop),m1),m2)),False)) -> begin
 ...
 | EqualProp(h,s,t,r1,r2) -> begin
   ...
   match p with  
     | Ap(Ap(Eq(a1),m1),m2) when a1 = mk_prop -> begin
            if  trm_eq (normalize m2)  t && trm_eq (normalize m1)  s then
 ...
 | NegEqualFunc(h,s,r) -> begin 
   ...
   match p with  
     | (Ap(Ap(Imp,Ap(Ap(Eq(aa'),m1),m2)),False)) when is_ar aa' -> 
	begin
          let a = ty_get_l aa' in
          let a' = ty_get_r aa' in
	  let left = (Ap(shift m1 0 1,DB(0,a))) in
	  let right =  (Ap(shift m2 0 1,DB(0,a))) in
	  let aao= mk_ar a' (mk_ar a' mk_prop) in
 ...
 | EqualFunc(h,s,r) -> begin 
   ...
   match p with  
   | Ap(Ap(Eq(aa'),m1),m2) when is_ar aa' -> begin
       let a = ty_get_l aa' in
       let a' = ty_get_r aa' in
       let left = (Ap(shift m1 0 1,DB(0,a))) in
	let right =  (Ap(shift m2 0 1,DB(0,a))) in
 ...
 | ChoiceR(eps,pred,s,t,r1,r2) -> 
    let a = match ty_pred_over (tpof pred) with Some(a) -> a | _ -> raise TranslateFailure (*** Sep 2011 - Chad : start over with safe_translate ***) (** failwith  "Error pred is not of type a --> o" **) in	
...
| Trans(h1,h2,su,r1) -> 
 ...
 | NegEqualProp(h,s,t,r1,r2) -> begin
   ...
   if debug_translation then Printf.printf  "boolean extensionality %s\n" (trm_str p);
   match p with  
     | (Ap(Ap(Imp,Ap(Ap(Eq(a1),m1),m2)),False)) when a1 = mk_prop -> (* not equal *)
		begin
            if  trm_eq (normalize m2)  t && trm_eq (normalize m1)  s then
 ...
 | EqualProp(h,s,t,r1,r2) -> begin
   ...
   if debug_translation then Printf.printf  "boolean Equality %s\n" (trm_str p);
   match p with  
     | Ap(Ap(Eq(a1),m1),m2) when a1 = mk_prop -> (* equal *)
		begin
            if  trm_eq (normalize m2)  t && trm_eq (normalize m1)  s then
 ...
 | ChoiceR(eps,pred,s,t,r1,r2) -> begin
	match eps with  
	| Name(x,a1) when is_ar a1 && is_ar (ty_get_l a1) && ty_get_r (ty_get_l a1) = mk_prop -> (* Case 1: The choice is a name defined by an axiom *)
...
let m1 = Ap (Forall (mk_ar a mk_prop),Lam (mk_ar a mk_prop,Ap (Forall a,Lam (a,Ap (Ap (Imp, Ap (DB (1, mk_ar a mk_prop), DB (0, a))),
	 Ap (DB (1, mk_ar a mk_prop),Ap (Name (x, mk_ar (mk_ar a mk_prop) a), DB (1, mk_ar a mk_prop)))))))) in
let m2 = Ap (Forall (mk_ar a mk_prop),Lam (mk_ar a mk_prop,Ap(Ap (Imp,Ap(Ap (Imp,Ap (Forall a,Lam (a,
	Ap (Ap (Imp, Ap (DB (1, mk_ar a mk_prop), DB (0, a))),False)))),False)),Ap (DB (0, mk_ar a mk_prop),
	Ap (Name (x, mk_ar (mk_ar a mk_prop) a),DB (0, mk_ar a mk_prop)))))) in
let m3 = Ap (Forall (mk_ar a mk_prop),Lam (mk_ar a mk_prop,Ap(Ap (Imp,Ap (Exists a,Lam (a,
	Ap (DB (1, mk_ar a mk_prop), DB (0, a))))),Ap (DB (0, mk_ar a mk_prop),
	Ap (Name (x, mk_ar (mk_ar a mk_prop) a),DB (0, mk_ar a mk_prop)))))) in
try
...
let rec safe_fill_translate r =
  ...
  | NegEqualFunc(Ap(Ap(Imp,Ap(Ap(Eq(ab),m1a),m1b)),False) as m0,m2,r1) when is_ar ab ->
      NegEqualFunc(m0,m2,safe_fill_translate r1)
  | EqualFunc(Ap(Ap(Eq(ab),m1a),m1b) as m0,m2,r1) when is_ar ab ->
      EqualFunc(m0,m2,safe_fill_translate r1)
  | ChoiceR(eps,pred,empty,choice,r1,r2) -> (*** Assume pred is normal ***)
      begin
	match ty_pred_over (tpof pred) with
	| Some(a) ->
	    (* TODO: why was m1 ever declared here? *)
