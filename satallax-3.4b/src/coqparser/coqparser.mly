...
documentitem:
  ...
  | SECTION ID DOT {
    ...
    else raise (Error.GenericError "Sections are only allowed in global Coq files.") }
  | END ID DOT {
    if (!State.coqglobalfile) then
      begin
	...
	| ((x,_,_)::_) -> raise (Error.GenericError ("Attempt to End Section " ^ $2 ^ ", but Sections " ^ x ^ " is open."))
	| _ -> raise (Error.GenericError "Attempt to End a Section, but no sections are open.")
      end
    else raise (Error.GenericError "Sections are only allowed in global Coq files.") }
  | HYP ID COLON preterm DOT {
    ...
    else raise (Error.GenericError "Hypotheses are only allowed in global Coq files.") }
  ...
  | CONJECTURE ID COLON preterm DOT {
	...
	begin
	  if (!Log.verbosity > 20) then (Printf.printf "call: "; List.iter (fun x -> Printf.printf " %s" x) (List.rev sargs); Printf.printf "\n");
  ...
  | VAR ID COLON stp DOT {
    ...
    else raise (Error.GenericError "Variables are only allowed in global Coq files.") }
  ...
  | BLET ID COLON stp DEQ preterm DOT {
    ...
    else raise (Error.GenericError "Lets are only allowed in global Coq files.") }
  ...
  | BLET ID tpids DEQ preterm DOT {
	...
	State.coqctx := (Some $2,None,Some pretm)::!State.coqctx
      ...
      raise (Error.GenericError "Lets are only allowed in global Coq files.") }
      ;
stp : stp1 { $1 }
