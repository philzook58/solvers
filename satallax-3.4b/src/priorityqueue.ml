...
(*** Version 2: Wasn't fair due to faulty implementation; deleted ***)
...
module PriorityQueue8 = functor (T : sig type t end) ->
 ...
 (*** Version 9: Balanced binary tree with same priorities treated as stacks (implemented as lists on the node of the priority). Fairness ensured with an offset incremented every 65536 inserts. ***)
module PriorityQueue9 = functor (T : sig type t end) ->
  struct
    type a = T.t
    let logmodulus = 16
    let pqueue_offset = ref 0
...
(*** Version 10: Balanced binary tree with same priorities treated as stacks (implemented as lists on the node of the priority). Fairness ensured with an offset incremented every 65536 inserts. ***)
module PriorityQueue10 = functor (T : sig type t end) ->
  struct
    type a = T.t
    let logmodulus = 16
    let pqueue_offset = ref 0
